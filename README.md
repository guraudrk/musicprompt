...
**(이어서, 속도/신뢰성 긴급 수정)** 이론 반영 배포 직후 사용자가 다시 Mock 템플릿 결과를 봤다며
"이론이 진짜 반영된 거 맞아?", "속도를 최소 3배는 줄여야겠다"고 알려줬습니다. 서버 로그를 확인해보니
실제 Gemini 호출이 90초 타임아웃에 걸려 개발 환경에서 조용히 Mock으로 폴백된 것이었고, 재시도 설정
때문에 최악의 경우 3~6분까지 걸리고 있었습니다. 세 가지를 고쳤습니다: (1) 데모에서는 화면에 표시도
안 되는 평가(evaluator) 단계를 완전히 Mock으로 고정해서 낭비되는 Gemini 호출 하나를 제거, (2) 재시도
횟수를 1회→0회로 줄여서 최악의 대기 시간을 절반으로, (3) 이론 설명 프롬프트를 약 40% 압축. **솔직한
검증 결과**: 완전히 다른 평범한 문장은 26.5초 만에 훌륭한 결과를 냈지만(기존 ~70초에서 크게 개선),
사용자가 처음 테스트했던 그 문장(한/일 이중 언어 + 남녀 대화 + 두 장르 융합)은 세 가지를 다 고친
뒤에도 여전히 가끔 90초 타임아웃에 걸렸습니다. 다른 문장은 바로 성공했으므로 세션 전체가 느려진 게
아니라 그 특정 문장이 모델에게 유난히 까다로운 요청이라고 판단했고, "모든 입력에 3배 빨라진다"고
과장하지 않고 일반적인 경우는 확실히 개선됐지만 유난히 까다로운 입력은 여전히 가끔 느릴 수 있다고
정직하게 남겨뒀습니다. 상세는 `docs/PHASE_LOG.md`의 "Demo latency/reliability fix" 항목과
`DECISIONS.md` ADR-049 참고.

**(이어서, 근본 원인 조사)** 사용자가 "임시방편 말고 최신 AI 기술로 속도와 품질을 동시에 해결하라"고
요구해서, LLM이 실제로 창작 판단이 필요 없는 필드(제공자 메타데이터, spec에서 기계적으로 뽑아낼 수
있는 theoryRationale, Stage F 평가자가 매번 덮어써서 어차피 버려지는 promptQuality 등 10개 필드)까지
같이 생성하고 있었다는 걸 발견했습니다. `CompilerOutputSchema`로 LLM이 생성해야 할 범위를 진짜 창작이
필요한 5개 필드로 좁히고, 나머지는 `src/compiler/deterministicFields.ts`에서 결정론적으로 조립하도록
리팩터링했습니다(ADR-050, 유닛 테스트 169개 통과). 그런데 라이브 검증 중 이전에 빨랐던 문장까지 계속
90초 타임아웃이 나서, "혹시 API가 잘 안 불러와지는 거 아니냐"는 사용자 질문에 직접 답하기 위해
API 키/설정 로딩부터 검증했습니다(정상 확인). 앱을 완전히 우회해 Gemini SDK를 직접 호출해 재현한
결과, 실제 시스템 지침과 실제 스키마를 같이 쓸 때만 180초를 기다려도 응답이 없이 멈추는 현상을
확인했고, 배열 필드에 상한(`.max()`)이 없는 게 원인일 수 있다는 단서를 찾아 축소 스키마로 재현
테스트하니 16.5초 만에 성공했습니다. 이 상한을 실제 스키마에 적용했지만(`theoryAddressal` 등), **정직한
검증 결과**: 적용 후 실제 데모로 재검증했을 때도 여전히 90초 타임아웃이 발생해 완전한 해결로 확인되지
않았습니다. 상한 자체는 합리적인 개선이라 유지하되, 근본 원인은 아직 부분적으로만 파악된 상태로
정직하게 남겨뒀고, 이미 상당한 실제 API 시간/비용을 쓴 만큼 추가 라이브 진단은 사용자 결정으로
잠정 중단했습니다. 상세는 `docs/PHASE_LOG.md`의 해당 항목과 `DECISIONS.md` ADR-050/ADR-051 참고.

**(이어서)** 사용자가 "이론 문서 전체를 통째로 넣자"는 대신 "입력 문장의 장르에 맞는 부분만 골라
넣자"는 더 나은 대안을 제안해서, 시스템 지침의 "장르별 안내" 부분을 실제 선언된 장르에 맞는 것만
넣도록 구현했습니다(`selectGenreTopline`, 미인식/무장르 시 기존처럼 전체 6개 장르 안내로 안전하게
폴백). 유닛 테스트 4개 추가, 173개 전체 통과. 그런데 "너무 많이 테스트하지 말라"는 요청에 따라 딱
한 번만 라이브 검증했는데, 이번엔 조용한 타임아웃이 아니라 **Google이 직접 보낸 에러**를 받았습니다:
`gemini-3.5-flash is currently experiencing high demand`. 즉 그동안 반복됐던 불안정성 상당 부분이
우리 코드/스키마 문제가 아니라 지금 이 시점에 Gemini 모델 자체가 겪고 있는 실제 수요 폭증 때문일
수 있다는 걸 시사합니다. 이번에도 Mock으로 폴백돼서 이 개선의 실제 품질 효과는 아직 확인 못 했고,
사용자 결정에 따라 추가 라이브 테스트는 멈췄습니다. 상세는 `DECISIONS.md` ADR-052 참고.
