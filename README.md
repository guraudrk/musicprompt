...
이틀 전(2026-07-15)에 남겨뒀던 두 가지 확인 작업 — 일반 문장에서 이론이 실제로 잘 반영되는지, 그리고
장르 타겟팅(ADR-052)이 효과가 있는지 — 을 이어서 진행했습니다. Gemini가 정상적으로 응답하는지 먼저
가볍게 핑 테스트로 확인한 뒤(4.6초, 정상), 데모 엔드포인트로 일반 문장을 실제 Gemini로 컴파일해봤고,
드디어 오늘 처음으로 Mock 폴백이 아닌 진짜 Gemini 응답을 받았습니다(22.8초, 서버 로그에 폴백 경고
없음 확인). 결과를 뜯어보니 "벌스는 조용하고 친밀하게, 코러스는 밝고 힘차게"처럼 우리 이론 문서의
긴장과 해소(tension-and-release) 원칙이 형용사 나열이 아니라 실제 문장 구조에 구체적으로 반영돼
있었습니다. 다만 좋은 점만 있던 건 아니고, 제목 필드에 곡 제목치고는 부자연스러운 메타 설명이
괄호로 섞여 들어가 있었고, 구간별 구조 설명(`structureNotes`)은 아예 비어 있었습니다.

사용자가 이 두 가지 결함부터 먼저 고치자고 해서, 시스템 지침에 (1) 제목은 해석 과정 설명 없이 깨끗한
곡 제목만 쓸 것, (2) 구조가 명확히 자유형식이 아닌 이상 구간별 구조 설명을 기본으로 채울 것을
명시적으로 추가했습니다. typecheck/lint/유닛 테스트(173개) 전부 통과했지만, 이 수정을 검증하려고
같은 문장으로 두 번 더 시도한 결과 둘 다 90초 타임아웃으로 Mock에 폴백돼 **이 수정의 실제 효과는
아직 라이브로 확인하지 못했습니다** — 확인된 것처럼 포장하지 않고 그대로 남겨뒀습니다. Gemini의
간헐적인 불안정성(지난 세션부터 이어짐)이 여전히 해소되지 않은 상태입니다. 상세는 `DECISIONS.md`
ADR-053 참고.

**(이어서)** 사용자가 "Gemini 응답 문제도 해결해야 돼"라고 명확히 요구해서, 외부 문제로 기록만 하고
넘기는 대신 실제로 고칠 수 있는 부분을 찾았습니다. Google이 명시적으로 "수요 폭증" 500 에러를 보낼
때, 그리고 조용한 타임아웃이 날 때(사용자에게 트레이드오프를 먼저 확인한 뒤 — 최악 대기 시간이
늘어나지만 짧은 예산(30초)으로 제한) 문서화된 대체 모델(`gemini-2.5-flash`)로 한 번 더 재시도하도록
만들었습니다. 라이브로 확인해보니 실제로 작동해서 한 번은 타임아웃 후 폴백 모델이 성공적으로
응답했습니다. 다만 그 과정에서 새 문제 두 가지를 더 발견했습니다: 폴백 모델이 가끔 마크다운
코드펜스(` ```json `)로 감싸서 응답하는 것(수정함), 그리고 폴백 모델이 우리 스키마를 완벽히 안
지켜서 필수 필드가 빠지는 경우(이건 더 근본적인 문제라 다음 세션으로 넘김). 유닛 테스트 8개
추가/수정, 전체 180개 통과. 상세는 `DECISIONS.md` ADR-054 참고.
